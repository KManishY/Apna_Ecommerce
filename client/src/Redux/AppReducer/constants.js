export const GET_DATA_REQUEST = "GET_DATA_REQUEST";
export const GET_DATA_SUCCESS = "GET_DATA_SUCCESS";
export const GET_DATA_FAIL = "GET_DATA_FAIL";

export const GET_CART_DATA_REQUEST = "GET_CART_DATA_REQUEST";
export const GET_CART_DATA_SUCCESS = "GET_CART_DATA_SUCCESS";
export const GET_CART_DATA_FAIL = "GET_CART_DATA_FAIL";

export const POST_CART_DATA_REQUEST = "POST_CART_DATA_REQUEST";
export const POST_CART_DATA_SUCCESS = "POST_CART_DATA_SUCCESS";
export const POST_CART_DATA_FAIL = "POST_CART_DATA_FAIL";


export const POST_CART_AllDATA_REQUEST = "POST_CART_AllDATA_REQUEST";
export const POST_CART_AllDATA_SUCCESS = "POST_CART_AllDATA_SUCCESS";
export const POST_CART_AllDATA_FAIL = "POST_CART_AllDATA_FAIL";


